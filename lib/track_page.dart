...
class _TrackPageState extends State<TrackPage> {
            ...
            child: Column(
              children: [
                const CircleAvatar(
                  ...
                  child: Icon(Icons.track_changes, size: 50, color: Colors.teal),
                ),
                const SizedBox(height: 10),
                const Text(
 ...
 Widget _buildDailySummary(BuildContext context) {
  ...
  //                 final key = entry.key; // Firestore key
  //                 final isCompleted = entry.value;
  //                 final taskName = provider.taskKeyMapping[key] ?? key; // Map Firestore key to user-friendly name
  //
  //                 return Container(
  ...
  //                       value: isCompleted,
  //                       onChanged: (value) {
  //                         provider.toggleTask(taskName, value); // Use user-friendly name
  //                         // Optional feedback
  //                         ScaffoldMessenger.of(context).showSnackBar(
  //                           SnackBar(
  //                             content: Text('$taskName marked as ${value ? "completed" : "pending"}'),
        ...
        ...tasks.map((task) {
          ...
          return Container(
            ...
            child: ListTile(
              contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 12),
              title: Text(
                formatTitle(task['title']) as String,
                style: TextStyle(
                  fontSize: 18,
                  fontWeight: FontWeight.w600,
                  color: isCompleted ? Colors.black : Colors.grey.shade600,
